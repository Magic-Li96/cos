package cos.servlet.departmentHead.article.search;
 ...
 */
@WebServlet("/findArticleForCourseIndex")
public class FindArticleForCourseIndex extends HttpServlet {
			...
			for (DepartmentHead departmentHead : departmentHeads) {
				if (departmentHead.getDepartmentHeadId().equals(user.getUserId())) {
					for (Article article : articles) {
						if (article.getCourse().equals(Integer.parseInt(courseId))) {
							if (article.getDepartment().equals(departmentHead.getDepartmentId())) {
								result.add(article);
							}
						}
					}
					...
					if (arrays.size()==0) {
						...
						response.sendRedirect("/cos/system/departmentHead/article.jsp?title=search");
						return;
					}
