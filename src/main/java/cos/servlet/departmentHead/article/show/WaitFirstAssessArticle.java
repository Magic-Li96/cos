...
/**
 * Servlet implementation class WaitFirstAssessArticle
 */
@WebServlet("/waitFirstAssessArticle")
public class WaitFirstAssessArticle extends HttpServlet {
			...
			for (DepartmentHead departmentHead : departmentHeads) {
				if(departmentHead.getDepartmentHeadId().equals(userId)) {
					...
					for (Article article : articles) {
						//选择待初审课程大纲加入结果集
						if (article.getDepartment().equals(departmentId)&&article.getContidion().equals(3)&&article.getSubject().equals(subId)) {
							result.add(article);
						}
					}
					// 确定分页总数
