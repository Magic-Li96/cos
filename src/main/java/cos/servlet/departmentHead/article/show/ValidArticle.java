...
/**
 * Servlet implementation class ValidArticle
 */
@WebServlet("/validArticle")
public class ValidArticle extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			request.getSession().setAttribute("vaA", new Object());
			...
			for (DepartmentHead departmentHead : departmentHeads) {
				if(departmentHead.getDepartmentHeadId().equals(userId)) {
					...
					for (Article article : articles) {
						//选择生效的课程大纲加入结果集
						if (article.getDepartment().equals(departmentId)&&article.getContidion().equals(6)&&article.getSubject().equals(subId)) {
							result.add(article);
						}
					}
					// 确定分页总数
