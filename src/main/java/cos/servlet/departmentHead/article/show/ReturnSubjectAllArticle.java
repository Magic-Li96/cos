 ...
 */
@WebServlet("/returnSubjectAllArticle")
public class ReturnSubjectAllArticle extends HttpServlet {
			...
			for (DepartmentHead departmentHead : departmentHeads) {
				if(departmentHead.getDepartmentHeadId().equals(userId)) {
					...
					for (Article article : articles) {
						//选择课程大纲加入结果集
						if (article.getDepartment().equals(departmentId)&&article.getSubject().equals(subId)) {
							result.add(article);
						}
					}
					//sqlSession.close();
