 ...
 */
@WebServlet("/insAllArticleForLeader")
public class InsAllArticleForLeader extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			for (Article article : articles) {
				...
				if (article.getStartInstitute().equals(user.getInstitute())) {
					result.add(article);
				}
			}
			...
			if (arrays.size() == 0) {
				...
				response.sendRedirect("/cos/system/leader/article.jsp?title='leaAllArt'");
				return;
			}
