 ...
 */
@WebServlet("/finalPassSuccess")
public class FinalPassSuccess extends HttpServlet {
		...
		try {
			...
			for (Article article : articles) {
				if (article.getArticleSrc()!=null&&article.getArticleSrc().equals(src)) {
					...
					message.setMessage("您的《"+article.getArticleName()+"》课程大纲终审已通过！");
					...
					sqlSession.commit();
					sqlSession.close();
					response.sendRedirect("/cos/waitFinallAccessArticle");
					return;
				}
			}
		} catch (Exception e) {
