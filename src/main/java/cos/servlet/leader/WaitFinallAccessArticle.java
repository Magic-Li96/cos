...
/**
 * Servlet implementation class WaitSecondAccessArticle
 */
@WebServlet("/waitFinallAccessArticle")
public class WaitFinallAccessArticle extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			request.getSession().setAttribute("wfA", new Object());
			...
			for (Article article : articles) {
				// 选择待复审课程大纲加入结果集
				if (article.getStartInstitute().equals(user.getInstitute()) && article.getContidion().equals(5)) {
					result.add(article);
				}
			}
			...
			if (arrays.size() == 0) {
				...
				response.sendRedirect("/cos/system/leader/article.jsp?title='待终审课程大纲'");
				return;
			}
