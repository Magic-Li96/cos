 ...
 */
@WebServlet("/waitSecondAccessArticle")
public class WaitSecondAccessArticle extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			request.getSession().setAttribute("wsA", new Object());
			...
			for (Article article : articles) {
				// 选择待复审课程大纲加入结果集
				if (article.getStartInstitute().equals(user.getInstitute()) && article.getContidion().equals(4)) {
					result.add(article);
				}
			}
			...
			if (arrays.size() == 0) {
				...
				response.sendRedirect("/cos/system/instituteOffice/article.jsp?title='待复审课程大纲'");
				return;
			}
